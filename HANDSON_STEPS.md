...
- Unity Catalog の `main` カタログに `bronze`、`silver`、`gold` スキーマを作成または利用できる権限
...
実行後、Databricks の `main.bronze` スキーマに View が作成されます。
...
実行後、Databricks の `main.silver` スキーマに Table が作成されます。
...
実行後、Databricks の `main.gold` スキーマに Table が作成されます。
...
```sql
select *
from main.gold.gold_order_status_summary
...
| `main.bronze` などに作成できない | Unity Catalog のカタログ、スキーマ作成権限を確認する |
...
- Databricks に `main.bronze`、`main.silver`、`main.gold` のオブジェクトが作成されている
